...
namespace lib_math {
	...
	unsigned int t_plane::clip_triangle_alt(
		const t_raw_triangle& triangle,
		      t_raw_triangle* clipped_tris,
		const m_coor_type eps
	) const {
		const m_point_type& base = get_point(); // plane origin point
		const m_vector_type& norm = triangle.get_normal();

		// plane-edge clipping
		//   N is plane normal
		//   P is some point in plane, dot(N, P) = 0
		//   Q1 above plane, d1 = dot(N, Q1 - P) > 0
		//   Q2 below plane, d2 = dot(N, Q2 - P) < 0
		//   I is intersection-point of (Q2 - Q1) with plane
		//   four cases depending on whether Q1/Q2 are "in" or "out"
		//
		//   I     = Q1      +  t *  (Q2      -  Q1    )
		//   I - P = Q1 - P  +  t * ((Q2 - P) - (Q1 - P))
		//
		//   dot(N, I - P) = dot(N, Q1 - P  +  t * ((Q2 - P) - (Q1 - P)))
		//   dot(N, I - P) = dot(N, Q1 - P)  +  t * (dot(N, Q2 - P) - dot(N, Q1 - P))
		//   dot(N, I - P) = d1  +  t * (d2 - d1)
		//              0  = d1  +  t * (d2 - d1)
		//
		//   t = d1 / (d1 - d2)
		//
		m_point_type above[6];
		m_point_type below[6];

		unsigned int i = 0;
		unsigned int j = 0;

		for (unsigned int n = 0; n < 3; n++) {
			const m_point_type& cur_vert = triangle.get_vertex((n + 0)    );
			const m_point_type& nxt_vert = triangle.get_vertex((n + 1) % 3);
			const m_vector_type cur_vect = cur_vert - base;
			const m_vector_type nxt_vect = nxt_vert - base;

			const m_coor_type cur_dist = cur_vect.inner_product(norm);
			const m_coor_type nxt_dist = nxt_vect.inner_product(norm);
			...
			if (lib_math::sign(cur_dist) != lib_math::sign(nxt_dist)) {
				// edge <n,n+1> crosses plane; calculate intersection point
				const m_coor_type int_dist = cur_dist / (cur_dist - nxt_dist);
				const m_point_type int_vert = cur_vert + (nxt_vert - cur_vert) * int_dist;
